...
7. `containerd config default | sudo tee /etc/containerd/config.toml` (cat /etc/containerd/config.toml)
8. set SystemdCgroup to true (`sudo nano /etc/containerd/config.toml`)
```
[plugins."io.containerd.grpc.v1.cri".containerd.runtimes.runc.options]
            ...
            Root = ""
            ShimCgroup = ""
            SystemdCgroup = true # 이 부분 true로 수정
```
9. disabled swap (`sudo nano /etc/fstab`)
...
```
10. uncomment the `net.ipv4.ip_forward=1` (`sudo nano /etc/sysctl.conf`)
```
...
11. create file and add following content (ensures briging is fully supported in the cluster) (`sudo nano /etc/modules-load.d/k8s.conf`)
...
do not change the `--pod-network-cidr` only set the `--control-plane-endpoint` to k8s control node ip
```
...
sudo kubeadm init --control-plane-endpoint=192.168.1.139 --node-name k8s-ctrl --pod-network-cidr=10.244.0.0/16

if the previous command was successful following ouput will comeout
```
...
```
sudo kubeadm join 192.168.1.139:6443 --token mlz4ks.gjiulql5r0ubk28y \
	--discovery-token-ca-cert-hash sha256:40817f088465bec3f5dd081c6e4697b2d6bca4b3e9d48444ac9751bcdfe87863
```
...
3. apply pod yml to cluster
```
kubectl apply -f pod.yml
kubectl apply -f service.yml
```

4. `curl $(kubectl get service | grep nginx-example | awk '{print $3}'):80`
